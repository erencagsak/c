...
export default function Topbar() {
  ...
  // For Developer
  const [devOpenWindow, setDevOpenWindow] = useState(false);
  const isLocalHost = typeof window !== "undefined" &&
  (window.location.hostname === "localhost" || window.location.hostname.startsWith("192.168.") || window.location.hostname === "localhost");
    ...
    <>
          ...
          <div className="flex items-center">
            <DropdownMenu>
              ...
              >
                ...
                {isLocalHost && (
                  <DropdownMenuItem
                    onClick={() => setDevOpenWindow(true)}
                    className="group rounded-lg px-4 py-3 cursor-pointer hover:bg-blue-500/10 focus:bg-blue-500/10 transition-all duration-200"
                  >
                    <Menu className="w-5 h-5 mr-3 text-blue-400 group-hover:scale-110 group-hover:text-blue-300 transition-all"/>
                    <span className="text-white/90 group-hover:text-white font-medium">Developer Tools</span>
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      {devOpenWindow && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-9999">
          <div className="bg-slate-900 p-6 rounded-xl border border-white/10 w-[380px] shadow-2xl">
            <h2 className="text-white text-xl font-semibold mb-4">Developer Tools</h2>

            <button
              onClick={async () => {
                const response = await fetch("/api/process-events");
                const json = await response.json();
                
                alert("Process Completed\n" + JSON.stringify(json,null,2));
              }}
              className="w-full px-4 py-3 bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/40 rounded-lb text-white transition font-semibold"
            >
              RUN - Process Events 
            </button>

            <button
              onClick={() => setDevOpenWindow(false)}
              className="mt-6 text-white/70 hover:text-white transition underline w-full text-center"
            >
              Close
            </button>
          </div>
